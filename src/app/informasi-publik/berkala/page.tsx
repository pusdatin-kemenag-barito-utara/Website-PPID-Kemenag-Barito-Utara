...
export default function InformasiBerkalaPage() {
  ...
  const berkalaDocs = docs.filter(
    (doc) => doc.category === "Berkala" && doc.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="w-full px-4 md:px-8 lg:px-12 py-10 space-y-10">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-950 via-[#007144] to-emerald-900 text-white p-8 md:p-12 shadow-xl">
        <div className="relative z-10 max-w-3xl space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-emerald-200 text-xs font-bold uppercase tracking-wider backdrop-blur-md">
            <Calendar className="w-4 h-4" /> Informasi Publik
          </div>
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight">Informasi Berkala</h1>
          <p className="text-emerald-100/90 text-sm md:text-base leading-relaxed">
            Informasi publik yang wajib disediakan dan diumumkan secara berkala oleh Kementerian Agama Barito Utara.
          </p>
        </div>
      </section>
      ...
      <section className="space-y-6">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          <h2 className="text-xl font-extrabold text-foreground">Daftar Berkas Informasi Berkala ({berkalaDocs.length})</h2>
        ...
        <div className="grid gap-4">
          {berkalaDocs.length > 0 ? (
            berkalaDocs.map((doc) => (
              <div key={doc.id} className="p-6 rounded-2xl bg-card border border-border/60 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-[#007144]/40 transition-all">
            ...
            <div className="p-12 text-center bg-card border border-border/60 rounded-2xl text-muted-foreground text-sm">
              Belum ada dokumen informasi berkala yang ditemukan.
            </div>
          )}
        </div>
      </section>
