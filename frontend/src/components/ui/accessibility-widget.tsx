...
import { trackAccessibility } from '@/lib/analytics';
import './AccessibilityWidget.css';

// ─── SVG Icons Matching Kemenag Official System ─────────────────────────────
...
// ─── Main AccessibilityWidget Component ─────────────────────────────────────
...
export default function AccessibilityWidget() {
	...
	// Screen Reader (Click-to-Read Indonesian Speech Synthesis) Logic
	useEffect(() => {
		if (
		...
		const handleClick = (e: MouseEvent) => {
			...
			if (
				target.closest('.a11y-drawer') ||
				target.closest('.a11y-tab') ||
				target.closest('.live-support-window')
			) {
				return;
			}
			...
			const textElem =
				target.closest('p, span, h1, h2, h3, h4, h5, h6, a, button, label, li, td, th') || target;
	...
	const toggle = useCallback((key: keyof A11ySettings) => {
		setSettings((p) => {
			const next = !p[key];
			trackAccessibility(String(key), next);
			return { ...p, [key]: next };
		});
	}, []);
	...
	const changeFontSize = useCallback((delta: number) => {
		setSettings((p) => {
			const next = Math.min(150, Math.max(80, p.fontSize + delta));
			trackAccessibility('fontSize', next);
			return { ...p, fontSize: next };
		});
	}, []);

	const resetAll = useCallback(() => {
		trackAccessibility('reset_all', true);
	...
	return (
		<>
			{/* ── Side Tab (Bertengger di Kiri Layar - Sama Seperti Website Kemenag) ── */}
			{!isOpen && (
				<button
					type="button"
					className="a11y-tab"
			...
			{isOpen && (
				<div
					className="a11y-backdrop animate-in fade-in duration-200"
					onClick={() => setIsOpen(false)}
					aria-hidden="true"
				/>
			)}

			{/* ── Drawer Panel (Meluncur dari Kiri Layar) ───────────────────────── */}
			<div
				...
				role="dialog"
				aria-label="Panel Aksesibilitas Difabel"
			>
				{/* Header */}
