...
export default function InformasiSertaMertaPage() {
  ...
  const sertaMertaDocs = docs.filter(
    (doc) => doc.category === "Serta Merta" && doc.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="w-full px-4 md:px-8 lg:px-12 py-10 space-y-10">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-amber-950 via-amber-800 to-emerald-950 text-white p-8 md:p-12 shadow-xl">
        <div className="relative z-10 max-w-3xl space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-amber-200 text-xs font-bold uppercase tracking-wider backdrop-blur-md">
            <AlertCircle className="w-4 h-4" /> Informasi Darurat & Mendadak
          </div>
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight">Informasi Serta Merta</h1>
          <p className="text-amber-100/90 text-sm md:text-base leading-relaxed">
            Informasi yang dapat mengancam hajat hidup orang banyak dan ketertiban umum yang diumumkan serta merta.
          </p>
        </div>
      </section>
      ...
      <section className="space-y-6">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          <h2 className="text-xl font-extrabold text-foreground">Daftar Berkas Informasi Serta Merta ({sertaMertaDocs.length})</h2>
        ...
        <div className="grid gap-4">
          {sertaMertaDocs.length > 0 ? (
            sertaMertaDocs.map((doc) => (
              <div key={doc.id} className="p-6 rounded-2xl bg-card border border-border/60 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-amber-500/40 transition-all">
                <div className="flex items-start gap-4">
                  <div className="p-3 rounded-2xl bg-amber-500/10 text-amber-700 shrink-0 mt-0.5">
                ...
                <button className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-amber-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold hover:bg-amber-800 active:scale-[0.98] transition-all shadow-xs shrink-0">
            ...
            <div className="p-12 text-center bg-card border border-border/60 rounded-2xl text-muted-foreground text-sm">
              Belum ada pengumuman informasi serta merta saat ini.
            </div>
          )}
        </div>
      </section>
